...
function crearGeometria(){
    ...
    switch (superficieADibujar) {
        ...
        case "tubosenoidal": 
            superficie3D = new TuboSenoidal(0.1,50,2,2);
        break;
...
function TuboSenoidal(amplitud,longitud, radio, altura){
    ...
    this.getPosicion=function(u,v){
        ...
        var x = radio * Math.cos(theta) + amplitud * Math.sin(longitud * v) * Math.cos(theta);
        var y = radio * Math.sin(theta) + amplitud * Math.sin(longitud * v) * Math.sin(theta);
        var z = v * altura;
        return [x,y,z];
    }
